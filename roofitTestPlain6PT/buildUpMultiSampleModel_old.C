...
//  if (nbins==14 || nbins==10 || nbins==5 || nbins==3 || nbins==15) useEta=true;
  ...
  for (int i=0;i<nJets;i++)
  {
    ...
    RooRealVar* weight=new RooRealVar(name,name,0.2,0.,1.);
...
//    ptCopyVars.push_back(pt);
  ...
  //  RooCategory* category=new RooCategory("sample","sample");
  //  category->defineType("QCDControlRegion",0);
  //  category->defineType("SignalRegion",1);

  //first for the isolation VARIABLE

  //I need N1*PDF_10*PDF_w1*PDF_W2*PDF_w3*PDF_w4
  //but only need PDF_10, PDF_wb, PDF_wc, PDF_wl
...
#include "btagBinning.icc"
 TArrayD limitsBWeighta(nLimitsBtag,limitsBWeight);
  ...
  if(tracks)
  {
    Double_t limitsPtEta1d_tracks[1000] = {ptmin,20e3,30e3,50e3,60e3,75e3,80e3,100e3,140e3,200e3,250e3,400e3};
    for(int i=0;i<20;i++)
    {
...
//      TH1F* histoBWeightLight=new TH1F(histoName5,histoName5,nbinsBTag,-0.5,nbinsNTag-0.5);
      ...
      for (int s=0;s<samplesToCreatePDFsfrom.at(i).size();s++)
      {
        ...
        tree->SetBranchAddress("bweight_mv2c20",&bweight_mv2c20);
	  ...
	  //          if((weightSingle)<0. && systematicsToLoad=="MCAtNLO") continue;
          ...
          if (channel>=4)
          {
            ...
            for (unsigned int p=0;p<2;p++)//need to consider both candidates
            {
              ...
              if (channel==4 || channel==5)
              {
                ...
                if ((bweight_mv2c20->at(1-p))<0.8119) 
                {
                  takejet[p]=false;
                }
              }
          ...
          if (channel==0 || channel==2 || channel==4 || channel==6)
          {
            ...
            if (channel==0)
            {
              if (first==second)
              {
                histoPtEta->Fill(ptcategory,weightSingle/2.);
                histoPtEta->Fill(ptcategory/nbins+(ptcategory%nbins)*nbins,weightSingle/2.);
              ...
              {
                histoPtEta->Fill(ptcategory,weightSingle);
                
                histoPtEtaJet0->Fill(ptcategory0,weightSingle);
                histoPtEtaJet1->Fill(ptcategory1,weightSingle);
              }
            }
            if (channel==2)
            {
              histoPtEtaJet0->Fill(ptcategory0,weightSingle);
              histoPtEtaJet1->Fill(ptcategory1,weightSingle);
              histoPtEtaJet2->Fill(ptcategory2,weightSingle);
              if (first==second)
              {
                histoPtEtaJet0->Fill(ptcategory1,weightSingle);
                histoPtEtaJet1->Fill(ptcategory0,weightSingle);
              }
              if (second==third)
              {
                histoPtEtaJet1->Fill(ptcategory2,weightSingle);
                histoPtEtaJet2->Fill(ptcategory1,weightSingle);
              }
              if (first==third)
              {
                histoPtEtaJet0->Fill(ptcategory2,weightSingle);
                histoPtEtaJet2->Fill(ptcategory0,weightSingle);
              }
            }
