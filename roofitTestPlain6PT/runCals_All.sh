...
#    echo "cd /afs/cern.ch/user/g/giacinto/work/HbbAnalysis/ElectroweakBosons_Feb2013;source scripts/setup_lxplus.sh;cd /afs/cern.ch/user/g/giacinto/work/HbbAnalysis/ElectroweakBosons_Feb2013/AnalysisWZorHbb/macros/ttbarBtagDileptonFit/roofitTestPlain6PT">>submitCals/$filename
    ...
    echo "root -b -q runGeneralFit.C\(0,true,false,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename
    echo "root -b -q runGeneralFit.C\(0,false,true,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename
    echo "root -b -q runGeneralFit.C\(1,true,false,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename
    echo "root -b -q runGeneralFit.C\(1,false,true,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename
    echo "root -b -q runGeneralFit.C\(2,true,false,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename
    echo "root -b -q runGeneralFit.C\(2,false,true,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename
    echo "root -b -q runGeneralFit.C\(3,true,false,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename
    echo "root -b -q runGeneralFit.C\(3,false,true,"$jobNumber",\\\"MV2c10"$workingpoint"\\\"\)" >> submitCals/$filename

#    qsub -N $(echo "run-"$jobNumber) -q medium $PWD/submitCals/$filename  
