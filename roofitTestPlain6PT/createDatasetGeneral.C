...
//      directory = (tracks) ? "./../../PGS_tracks/outputGeneral_fit_data_rescaling.root/tree_" : "/unix/atlas4/abell/FirstxAOD/abell/ttbarPDFCalibration/mc15data5/PGS/outputGeneral_fit_data_rescaling.root/tree_";
      directory = (tracks) ? "./../../PGS_tracks/outputGeneral_fit_data_rescaling.root/tree_" : "./../../PGS/outputGeneral_fit_data_rescaling.root/tree_";
...
//      std::cout << "weight after " << weight << std::endl;
    ...
    if(optimisation)
    {
      float leading_pt = (channel==0||channel==1) ? (pT->at(0)*(pT->at(0)>pT->at(1)) + pT->at(1)*(pT->at(1)>pT->at(0))) : (pT->at(0)*((pT->at(0)>pT->at(1))*(pT->at(0)>pT->at(2))) + pT->at(1)*((pT->at(1)>pT->at(0))*pT->at(1)>pT->at(2)) + pT->at(2)*((pT->at(2)>pT->at(0))*(pT->at(2)>pT->at(1))));
      if(channel==0)
      {
	if(leading_pt<60e3 && imbalance>0.45) { continue; }
	else if(leading_pt>60e3 && leading_pt<90e3 && (imbalance>0.55)) { continue; }
	else if(leading_pt>90e3 && leading_pt<140e3 && (delta_phi<0.75||imbalance>0.65)) { continue; }
	else if(leading_pt>140e3 && leading_pt<200e3 && (delta_phi<1.4||imbalance>0.72)) { continue; }
	else if(leading_pt>200e3 && leading_pt<300e3 && (delta_phi<1.5||imbalance>0.82)) { continue; }
      }
      else if(channel==1)
      {
	if(leading_pt<60e3 && imbalance>0.45) { continue; }
	else if(leading_pt>60e3 && leading_pt<90e3 && (imbalance>0.57)) { continue; }
	else if(leading_pt>90e3 && leading_pt<140e3 && (imbalance>0.71||delta_phi<0.5)) { continue; }
	else if(leading_pt>140e3 && leading_pt<200e3 && (imbalance>0.78||delta_phi<0.75)) { continue; }
	else if(leading_pt>200e3 && leading_pt<300e3 && (imbalance>0.88||delta_phi<1)) { continue; }
      }
      else if(channel==2)
      {
	if(leading_pt<60e3 && imbalance>1) { continue; }
	else if(leading_pt>60e3 && leading_pt<90e3 && (imbalance>0.56)) { continue; }
	else if(leading_pt>90e3 && leading_pt<140e3 && (delta_phi<0.75||imbalance>0.65)) { continue; }
	else if(leading_pt>140e3 && leading_pt<200e3 && (delta_phi<0.9||imbalance>1)) { continue; }
	else if(leading_pt>200e3 && leading_pt<300e3 && (delta_phi<0.9||imbalance>1)) { continue; }
      }
      else if(channel==3)
      {
	if(leading_pt<60e3 && imbalance>0.4) { continue; }
	else if(leading_pt>60e3 && leading_pt<90e3 && (delta_phi<0||imbalance>0.5)) { continue; }
	else if(leading_pt>90e3 && leading_pt<140e3 && (delta_phi<0||imbalance>0.66)) { continue; }
	else if(leading_pt>140e3 && leading_pt<200e3 && (delta_phi<0.3||imbalance>0.75)) { continue; }
	else if(leading_pt>200e3 && leading_pt<300e3 && (delta_phi<0.5||imbalance>0.82)) { continue; }
      }
    }
